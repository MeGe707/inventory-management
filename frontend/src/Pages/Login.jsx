  ...
  return (
    ...
    <form  onSubmit={onSubmitHandler} className=' min-h-[80vh] flex items-center'>
      <div className='bg-[#F2F3FF] flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg'>
        <p className='text-2xl  font-semibold'>{state === 'User Log In' ? "User Login" : "Admin Login"}</p>
        ...
        {
          state === "User Log In"
            ? <p>For admin account? <span onClick={() => setState('Admin Log In')} className='text-primary underline cursor-pointer'>Login here</span></p>
            : <p>For user account? <span onClick={() => setState('User Log In')} className='text-primary underline cursor-pointer'>Click here</span></p>
        }
